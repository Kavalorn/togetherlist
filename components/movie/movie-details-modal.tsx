...
export function MovieDetailsModal() {
  ...
  const { data: friendsWhoWatched, isLoading: isLoadingFriends } =
    ...
    const {
      ...
      getTitle,
      getDescription
    } = useMovieTranslations(selectedMovie?.id || null, selectedMovie?.overview, selectedMovie?.title);
  ...
  <Button 
    ...
    onClick={(e) => {
      e.preventDefault();
      e.stopPropagation();
      window.open(`https://www.google.com/search?q=${encodeURIComponent(getTitle())} ${selectedMovie.release_date}`, '_blank');
    }}
