...
//   async uakinoBay(title: string, year?: string | null): Promise<ServiceResult> {
//     const encodedTitle = encodeURIComponent(title.toLowerCase());
//     const searchUrl = `https://uakino-bay.net/search/${encodedTitle}`;
    
//     console.log(`Пошук на uakino-bay: ${searchUrl}`);
...
//     if (!response.ok) {
//       throw new Error(`Помилка запиту до uakino-bay: ${response.status}`);
//     }
...
//         const url = $(el).find('.short-img').attr('href');
//         const title = $(el).find('.short-title').text();
//         return { url, title };
//     }).get()[0] || {
//         url: null,
...
//   },
  ...
  async eneyida(title: string, year?: string | null): Promise<ServiceResult> {
    ...
    try {
        ...
        // Сортуємо результати за оцінкою співпадіння (від найбільшої до найменшої)
  ...
  async uafix(title: string, year?: string | null): Promise<ServiceResult> {
    try {
      ...
      // Встановлюємо власний таймаут для уникнення зависання при повільному відгуку
      ...
      // беремо з пешого діва з id dle-content - перший дочірній тег a, з нього беремо href. також з цього тегу беремо дочырный елемент з классом sres-text і з нього дочірній елемент h2 і беремо текст. на виході формуємо обьект з href і title
